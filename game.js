...
class Game {
    ...
    updateGame() {
        ...
        if (this.finished) {
            ...
            let message =
                config.settings.fixedLength && this.score < config.settings.scoreToWin
                    ? config.settings.timeUpText
                    : config.settings.winText;
    ...
    drawBoard() {
        ...
        if (item) {
            ...
            noFill();
            rect(
                start.x + (tileSize + tileOffset) * item.column - tileSize / 2,
                start.y + (tileSize + tileOffset) * item.row - tileSize / 2,
                tileSize,
                tileSize
            );
        }
    ...
    handleMatches() {
        ...
        gemsToRemove.map((gem) => {
            ...
            let ft = new FloatingText(
                config.settings.correctMovePoints,
                x + tileSize / 2,
                y + tileSize / 2,
                { x: random(-1, 1), y: -3 },
                random(30, 40),
                color(config.settings.onboardTextColor)
            );
